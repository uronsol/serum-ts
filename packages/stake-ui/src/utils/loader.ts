...
const UPGRADEABLE_LOADIER_INSTRUCTION_LAYOUT: Layout<UpgradeableLoaderInstruction> = rustEnum(
  [
...
export function encodeInstruction(i: UpgradeableLoaderInstruction): Buffer {
  ...
  const len = UPGRADEABLE_LOADIER_INSTRUCTION_LAYOUT.encode(i, buffer);
  return buffer.slice(0, len);
}

const UPGRADEABLE_LOADIER_STATE_LAYOUT: Layout<any> = rustEnum([
...
export function decodeState(data: Buffer): any {
  return UPGRADEABLE_LOADIER_STATE_LAYOUT.decode(data);
}
